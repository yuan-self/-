...
options.type = 'MLP1';
options.case = 'LOS';
options.expTag = ['G:/xiongjiyuan/6/sub/图/32天线不同数据量/' options.type '-' options.case 'tianxian'];
options.top_n = 3;
...
for ant = 1:length(num_ant_mm)% number of antennas to loop over
	fprintf('number of mmWave antennas: %d\n', num_ant_mm(ant))
    [W,~] = UPA_codebook_generator(1,num_ant_mm(ant),1,1,1,1,0.5);% Beam codebook
	options.codebook = W;
	options.numAnt = [num_ant_mm(ant)];
	options.numSub = 64;%子载波数
	options.inputDim = 3;
	options.valPer = 0.3;
	options.inputSize = [1,1,options.inputDim];
	options.noisyInput = false;
	options.bandWidth = 0.5;
	options.dataFile1 ='G:/xiongjiyuan/6/数据/16wireless_dataset.mat';% The path to the sub-6 data file
    options.dataFile2 ='G:/xiongjiyuan/6/数据/16wireless_dataset - 副本.mat';% The path to the mmWave data file
	...
	for p = 1:length(tx_power)
        ...
        for user = 1:size(X,4)
            ...
            % Top-3 average rate
            rec_power = abs( H'*W(:,three_best_beams) ).^2 * options.snr;
            rate_per_sub = log2( 1+rec_power );
            ave_rate_per_beam = mean(rate_per_sub,1);
...
variable_name = [options.expTag '0.1_results'];
